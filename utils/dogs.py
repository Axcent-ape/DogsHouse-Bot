...
class DogsHouse:
    ...
    async def timer(self):
        resp = await self.session.get(f'https://api.onetime.dog/advent/calendar/timer?user_id={self.telegram_id}')
        return (await resp.json()).get('Time')

    async def check_calendar(self, day: int):
        resp = await self.session.post(f'https://api.onetime.dog/advent/calendar/check?user_id={self.telegram_id}&day={day}')

        if resp.status != 200:
            logger.warning(f"Thread {self.thread} | {self.account} | Couldn't check day {day}: {(await resp.text()).strip()}")
            return False

        return resp.status == 200

    async def get_calendar(self):
        resp = await self.session.get(f'https://api.onetime.dog/advent/calendar?user_id={self.telegram_id}')

        if resp.status != 200:
            logger.warning(f"Thread {self.thread} | {self.account} | Couldn't get calendar: {(await resp.text()).strip()}")
            return False

        days = await resp.json()
        return next(day for day in days if day["IsCurrent"] and day["IsAvailable"])

    async def set_first_visit(self):
        resp = await self.session.post(f'https://api.onetime.dog/advent/calendar/first-visit/set?user_id={self.telegram_id}')
        return resp.status == 200

    async def first_visit(self):
        resp = await self.session.get(f'https://api.onetime.dog/advent/calendar/first-visit?user_id={self.telegram_id}')
        return (await resp.json()).get('FirstVisit')
